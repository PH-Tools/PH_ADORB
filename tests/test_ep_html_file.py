...
@pytest.mark.filterwarnings("ignore::DeprecationWarning")
def test_load_construction_quantities_data():
    """Test loading the 'Construction Quantities' table data."""
    _source_file_path = Path("tests/_test_input/example_annual_tables.htm")
    file_data = DataFileEPTables(source_file_path=_source_file_path, loader=load_construction_quantities_data)
    ...
    file_data.load_file_data()
    assert file_data.data is not None
    assert len(file_data.data) == 51


def test_malformed_construction_quantities_htm_file():
    # -- Create a temporary fake HTML file without the right table
    fake_html_file = Path("tests/_test_input/fake.htm")
    ...
    file_data = DataFileEPTables(source_file_path=fake_html_file, loader=load_construction_quantities_data)
