...
Finished feature/refactor folders (and single-file plans) folded back into `context/`, kept for history. Move an item here (unchanged) when its work is `Complete` or `Superseded`; keep the flat `<slug>` name so it stays findable by name.
